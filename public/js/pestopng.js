var color_def = [
 ...
 onmessage = async function(e) {
  ...
  try {
    var ab = await e.data[0].arrayBuffer();
    var data = new Uint8Array(ab);
    console.log(data);
    //parse PES
    var pesdata = parsePes(data, pes, my_colors);
    let canvaskontext = await printpng(pesdata, my_colors); 
    //OffsideCanvas umwandeln in gewünschtes Format -> DataURL
    let blob = await canvaskontext.canvas.convertToBlob();
    console.log(blob);
  ...
  while (p <= end) {
    ...
    if (val1 == 255 && val2 == 0) {
      ...
      if (val1 == 254 && val2 == 176) {
        ...
        if (val1 & 128) {
          //Das heißt der höchste Bit muss ne 1 sein-> führt zu 12 Bit Offset
          val1 = ((val1 & 15) << 8) + val2;
          if (val1 & 2048) {
            //das ist das fünfte Bit
            val1 -= 4096;
          }
          stitchtype = 1;
          val2 = data[p];
...
async function printpng(pesdata, my_colors) {
  ...
  let canvas = new OffscreenCanvas(x,y);
  ...
  for (var z = 0; z < blockchain.length; z++) {
    ...
    context.moveTo(
      (colorchain[0].x - pesdata.minx) * scale,
      (colorchain[0].y - pesdata.miny) * scale
    );
    for (var i = 1; i < colorchain.length - 1; i++) {
      var to = colorchain[i];
      if (to.stitchtype == 0) {
        context.lineTo(
          (to.x - pesdata.minx) * scale,
          (to.y - pesdata.miny) * scale
        );
      } else {
        context.moveTo(
          (to.x - pesdata.minx) * scale,
